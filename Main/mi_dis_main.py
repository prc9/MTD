...
import warnings
import numpy as np
from Model import mi_dis_model
from Model import simple_mi_dis_model
from Train import mi_dis_train
...
from Data.load_dataset5 import dataset5
from Data.load_dataset6 import dataset6
import get_miRNA_feature
from hetero_graph.mi_dis_hetero_graph import get_hetero_graph
# from get_disease_feature import get_disease_features
...
if __name__ == '__main__':
    ...
    mirna_num, target_num, disease_num, mirna_disease, target_disease, mirna_target, mirna_name, target_name, disease_name = dataset5()
    # mirna_num, target_num, disease_num, mirna_disease, target_disease, mirna_target, mirna_name, target_name, disease_name = dataset6()
    ...
    # disease_feat = get_disease_features(disease_num,feat_shape)
    # disease_feat = torch.tensor(disease_feat, dtype=torch.float32)
    ...
    print("Sparsity of miRNA-disease associated data: {:.6f}\n".format(
        len(mirna_disease[0]) / (mirna_num * disease_num)))

    edge_data = []
    for i in range(len(mirna_disease[0])):
        x = []
        x.append(mirna_disease[0][i])
        x.append(mirna_disease[1][i])
        edge_data.append(x)
    ...
    for train_index, test_index in kf.split(edge_data):
        ...
        G = mi_dis_model.generator(disease_num,feat_shape,out_feat)
        D = mi_dis_model.discriminator(disease_num,feat_shape,out_feat)
        ...
        Noise_MTDN, Noise_MTDN_h, True_MTDN, True_MTDN_h = get_hetero_graph(input_net, true_input_net, mirna_target,
                                                                            target_disease, mirna_feat, target_feat,
                                                                            disease_feat)

        auc = mi_dis_train.main(mirna_num, disease_num, epochs, pro_ZR, pro_PM, alpha, batchSize,
