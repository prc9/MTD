...
import warnings
import numpy as np
from Model import tar_dis_model
from Train import tar_dis_train
...
from hetero_graph.tar_dis_hetero_graph import get_hetero_graph
...
if __name__ == '__main__':
    ...
    print("Sparsity of miRNA-disease associated data: {:.6f}\n".format(
        len(target_disease[0]) / (target_num * disease_num)))

    edge_data = []
    for i in range(len(target_disease[0])):
        x = []
        x.append(target_disease[0][i])
        x.append(target_disease[1][i])
        edge_data.append(x)
    edge_data = np.array(edge_data)

    negativeSample_edge = []
    for i in range(len(edge_data)):
        row = random.randint(0, target_num - 1)
    ...
    for train_index, test_index in kf.split(edge_data):
        ...
        G = tar_dis_model.generator(disease_num,feat_shape,out_feat)
        D = tar_dis_model.discriminator(disease_num,feat_shape,out_feat)
        Noise_MTDN, Noise_MTDN_h, True_MTDN, True_MTDN_h = get_hetero_graph(input_net, true_input_net, mirna_target,
                                                                            mirna_disease, mirna_feat, target_feat,
                                                                            disease_feat)

        auc = tar_dis_train.main(target_num, disease_num, epochs, pro_ZR, pro_PM, alpha, batchSize,
