...
def get_hetero_graph(input_net, true_input_net, mirna_disease, target_disease, mirna_feat, target_feat, disease_feat):
    Noise_MTDN_data = {
        ('MiRNA', 'MvsT', 'Target'): (torch.tensor(input_net[0]), torch.tensor(input_net[1])),
        ('Target', 'TvsM', 'MiRNA'): (torch.tensor(input_net[1]), torch.tensor(input_net[0])),
    ...
    True_MTDN_data = {
        ('MiRNA', 'MvsT', 'Target'): (torch.tensor(true_input_net[0]), torch.tensor(true_input_net[1])),
        ('Target', 'TvsM', 'MiRNA'): (torch.tensor(true_input_net[1]), torch.tensor(true_input_net[0])),
        ('MiRNA', 'MvsD', 'Disease'): (torch.tensor(mirna_disease[0]), torch.tensor(mirna_disease[1])),
        ('Disease', 'DvsM', 'MiRNA'): (torch.tensor(mirna_disease[1]), torch.tensor(mirna_disease[0])),
        ('Target', 'TvsD', 'Disease'): (torch.tensor(target_disease[0]), torch.tensor(target_disease[1])),
        ('Disease', 'DvsT', 'Target'): (torch.tensor(target_disease[1]), torch.tensor(target_disease[0]))
