...
def get_hetero_graph(input_net, true_input_net, mirna_target, mirna_disease, mirna_feat, target_feat, disease_feat):
    Noise_MTDN_data = {
        ('Target', 'TvsD', 'Disease'): (torch.tensor(input_net[0]), torch.tensor(input_net[1])),
        ('Disease', 'DvsT', 'Target'): (torch.tensor(input_net[1]), torch.tensor(input_net[0])),
    ...
    True_MTDN_data = {
        ('Target', 'TvsD', 'Disease'): (torch.tensor(true_input_net[0]), torch.tensor(true_input_net[1])),
        ('Disease', 'DvsT', 'Target'): (torch.tensor(true_input_net[1]), torch.tensor(true_input_net[0])),
        ('MiRNA', 'MvsT', 'Target'): (torch.tensor(mirna_target[0]), torch.tensor(mirna_target[1])),
        ('Target', 'TvsM', 'MiRNA'): (torch.tensor(mirna_target[1]), torch.tensor(mirna_target[0])),
        ('MiRNA', 'MvsD', 'Disease'): (torch.tensor(mirna_disease[0]), torch.tensor(mirna_disease[1])),
        ('Disease', 'DvsM', 'MiRNA'): (torch.tensor(mirna_disease[1]), torch.tensor(mirna_disease[0]))
