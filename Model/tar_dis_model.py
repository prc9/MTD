...
class generator(nn.Module):
    ...
    def forward(self, g, h, Adj, size, leftIndex):
        ...
        target_feat = h3.get('Target')
        target_feat = torch.nn.functional.normalize(target_feat,p=1, dim=1)
        target_feat = torch.tensor(target_feat)

        fake_embeding = Variable(copy.deepcopy(target_feat[leftIndex:leftIndex + size]))
