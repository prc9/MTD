...
def main(target_num, disease_num, epochCount, pro_ZR, pro_PM, alpha, batchSize,
    ...
    Noise_TDA = adj(target_num, disease_num, input_net)
    True_TDA = adj(target_num, disease_num, true_input_net)
    Test_Adj = adj(target_num, disease_num, test_input_net)

    for epoch in range(epochCount):
        for step in range(G_step):
            leftIndex = random.randint(0, target_num - batchSize - 1)
            realData = Variable(copy.deepcopy(True_TDA[leftIndex:leftIndex + batchSize]))
            noiseData = Variable(copy.deepcopy(Noise_TDA[leftIndex:leftIndex + batchSize]))

            e_i = Variable(copy.deepcopy(Noise_TDA[leftIndex:leftIndex + batchSize]))
        ...
        for step in range(D_step):
            leftIndex = random.randint(1, target_num - batchSize - 1)
            realData = Variable(copy.deepcopy(True_TDA[leftIndex:leftIndex + batchSize]))
            noise_Data = Variable(copy.deepcopy(Noise_TDA[leftIndex:leftIndex + batchSize]))
        ...
        for testUser in range(len(Test_Adj)):
            data = Variable(copy.deepcopy(Noise_TDA[testUser:testUser + 1]))
