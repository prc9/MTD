...
def select_negative_items(Data, num_pm, num_zr, target_num):
    data = np.array(Data)
    n_items_pm = np.zeros_like(data)
    n_items_zr = np.zeros_like(data)
    for i in range(data.shape[0]):
        p_items = np.where(data[i] != 0)[0]
        all_item_index = random.sample(range(data.shape[1]), target_num)
...
def freeze_parameters(model, layers_to_freeze):
...
def main(mirna_num, target_num, epochCount, pro_ZR, pro_PM, alpha, batchSize,
    ...
    Noise_MTA = adj(mirna_num, target_num, input_net)
    True_MTA = adj(mirna_num, target_num, true_input_net)
    Test_Adj = adj(mirna_num, target_num, test_input_net)
    ...
    # 冻结生成器和判别器的部分层
    layers_to_freeze = ['HeteroConv1',]
    G = freeze_parameters(G, layers_to_freeze)
    D = freeze_parameters(D, ['f1'])


    for epoch in range(epochCount):
        for step in range(G_step):
            leftIndex = random.randint(0, mirna_num - batchSize - 1)
            realData = Variable(copy.deepcopy(True_MTA[leftIndex:leftIndex + batchSize]))
            noiseData = Variable(copy.deepcopy(Noise_MTA[leftIndex:leftIndex + batchSize]))

            e_i = Variable(copy.deepcopy(Noise_MTA[leftIndex:leftIndex + batchSize]))
            n_items_pm, n_items_zr = select_negative_items(noiseData, pro_PM, pro_ZR, target_num)
        ...
        for step in range(D_step):
            ...
            realData = Variable(copy.deepcopy(True_MTA[leftIndex:leftIndex + batchSize]))
            noise_Data = Variable(copy.deepcopy(Noise_MTA[leftIndex:leftIndex + batchSize]))

            e_i = Variable(copy.deepcopy(Noise_MTA[leftIndex:leftIndex + batchSize]))
            n_items_pm, _ = select_negative_items(noise_Data, pro_PM, pro_ZR, target_num)
        ...
        for testUser in range(len(Test_Adj)):
            data = Variable(copy.deepcopy(Noise_MTA[testUser:testUser + 1]))
